...
\begin{theorem}[\customlabel{coordinateFarkas}{\texttt{coordinateFarkas}}]
Let $I$ be any type.
...
\begin{theorem}[\customlabel{scalarFarkas}{\texttt{scalarFarkas}}]
Let $J$ be a finite type.
Let $R$ be a linearly ordered division ring.
...
a \emph{solution} to $P$ if and only if $A \* x \le b$.
We say that $P$ \emph{reaches} an objective value $r$
if and only if there exists $x$ such that $x$ is a solution to $P$
and $c \* x = r$.
We say that $P$ is \emph{feasible} if and only if $P$ reaches a finite\footnote{
...
reaching a finite value. {\color{blue} TODO this changed.}} value.
We say that $P$ is \emph{bounded by} a finite value $r$ if and only if,
for every value $p$ reached by $P$, we have $r \le p$.
We say that $P$ is \emph{unbounded} if and only if there is no finite value $r$
such that $P$ is bounded by $r$.
We say that the linear program $(-A^T, c, b)$ is the \emph{dual} of $P$.
\end{definition}

\begin{theorem}[\customlabel{ExtendedLP.weakDuality}{\texttt{ExtendedLP.weakDuality}}]
Let $F$ be a linearly ordered field.
...
Let $F$ be a linearly ordered field.
Let $P$ be a linear program over $F_\infty$.
We define the \emph{optimum} of $P$ as follows.
...
However, because we do not have the theorem now, the optimum is defined as
a partial function from linear programs to $F_\infty$.}
\end{definition}
...
\begin{theorem}[\customlabelrestated{weakDuality:restated}{\texttt{weakDuality}}]
Let $F$ be a linearly ordered field.
Let $P$ be a linear program over $F_\infty$.
If $P$ reaches $p$ and the dual of $P$ reaches $q$,
then $p + q \ge 0$.
