...
Two well-known theorems of alternative are as follows
(Farkas~\cite{Farkas1894,Farkas1898}; Minkowski~\cite{Minkowski}).

\begin{theorem}[\customlabel{equalityFarkas}{{\bf equalityFarkas}}]
...
\begin{theorem}[\customlabel{inequalityFarkas}{{\bf inequalityFarkas}}]
Let $I$ and $J$ be finite types.
Let $F$ be a linearly ordered field.
Let $A$ be a matrix of type $(I \times J) \r F$.
Let $b$ be a vector of type $I \r F$.
...
For optimization problems, duality asserts that primal and dual problems
have the same optimal values {\color{blue} TODO this sentence needs changes.}
(von Neumann~\cite{vonNeumann}; Gale, Kuhn, Tucker~\cite{GaleKuhnTucker}).
For example, for Linear Programs in the standard form, the strong duality theorem,
the cornerstone of linear programming, can be stated as follows.

\begin{theorem}[\customlabel{StandardLP.strongDuality}{{\bf StandardLP.strongDuality}}]
Let $I$ and $J$ be finite types.
Let $F$ be a linearly ordered field.
Let $A$ be a matrix of type $(I \times J) \r F$.
Let $b$ be a vector of type $I \r F$.
...
\begin{itemize}
\item We formally prove several existing duality results (including the statements above) in Lean 4.
In fact, we prove a more general version of \ref{equalityFarkas} due
to Bartl~\cite{Bartl2007}.
...
\end{itemize}
Sections~\ref{generalizations} and~\ref{extensions} provide further details
...
\begin{theorem}[\customlabel{coordinateFarkas}{{\bf coordinateFarkas}}]
Let $I$ be any type.
...
\end{itemize}
\end{theorem}
{\color{blue} TODO: describe how it implies \ref{equalityFarkas}?

This is so trivial that I don't know how to answer your question in a style
that is consistent with how the Introduction is written.}
...
\begin{theorem}[\customlabel{scalarFarkas}{{\bf scalarFarkas}}]
Let $J$ be a finite type.
Let $R$ be a linearly ordered division ring.
...
\sekt{modules}). This result is due to \cite{Bartl2007} and our proof is based on
\cite{Bartl2011}.

\begin{theorem}[\customlabel{fintypeFarkasBartl}{{\bf fintypeFarkasBartl}}]
Let $J$ be a finite type.
Let $R$ be a linearly ordered division ring.
Let $W$ be an $R$-module.
...
%
%\begin{definition}
%Let $I$ and $J$ be finite types, and $F$ be a linearly ordered field.
%Consider matrix $U:(I \times J) \r F_\infty$ and vector $u:I \r F_\infty$.
%The pair $(U,u)$ is called {\em nonconflicting} if (i) matrix $(U; u)$ does not have $\bot$ and $\top$
%in the same row, and (ii) matrix $U$ does not have $\bot$ and $\top$ in the same column.
%\end{definition}
%
%Condition for \ref{extendedFarkas}: Assume that pair $(A, -b)$ is nonconflicting.
%
%Condition for extended LP: Assume that pairs $(A, b)$ and $(A^T, c)$ are nonconflicting.
%
%Yet another presentation:
%\begin{definition}
%Consider matrix $A:(I \times J) \r F_\infty$ where $I,J$ are some types.
%A row (resp. colum) of $A$ is called {\em conflicting} if it contains both $\bot$ and $\top$.
%Otherwise it is {\em nonconflicting}.
%\end{definition}
%
%Condition for \ref{extendedFarkas}: Assume that pair all rows $(A\; -\!b)$ are nonconflicting
%and all columns of $A$ are nonconflicting.
%
%Condition for extended LP: Assume that all rows of $(A\; -\!b)$ 
%are nonconflicting and all columns of and $\begin{pmatrix}A \\ c\end{pmatrix}$ are nonconflicting.
%}

\begin{theorem}[\customlabel{extendedFarkas}{{\bf extendedFarkas}}]
Let $I$ and $J$ be finite types.
Let $F$ be a linearly ordered field.
...
\begin{theorem}[\customlabel{ExtendedLP.weakDuality}{{\bf ExtendedLP.weakDuality}}]
Let $F$ be a linearly ordered field.
...
\begin{theorem}[\customlabel{ExtendedLP.strongDuality}{{\bf ExtendedLP.strongDuality}}]
Let $F$ be a linearly ordered field.
Let $P$ be a linear program over $F_\infty$.
...
In \sekt{duality}, we prove \ref{ExtendedLP.strongDuality}
...
\begin{theorem}[\customlabel{finFarkasBartl}{{\bf finFarkasBartl}}]
...
\begin{lemma}[\customlabel{industepFarkasBartl}{{\bf industepFarkasBartl}}]
...
\begin{theorem}[\customlabel{inequalityFarkas-neg}{{\bf inequalityFarkas\_neg}}]
Let $I$ and $J$ be finite types.
Let $F$ be a linearly ordered field.
Let $A$ be a matrix of type $(I \times J) \r F$.
Let $b$ be a vector of type $I \r F$.
...
\begin{theorem}[\customlabelrestated{extendedFarkas:restated}{{\bf extendedFarkas}}]
Let $I$ and $J$ be finite types.
Let $F$ be a linearly ordered field.
Let $A$ be a matrix of type $(I \times J) \r F_\infty$.
Let $b$ be a vector of type $I \r F_\infty$.
...
\begin{theorem}[\customlabelrestated{weakDuality:restated}{{\bf weakDuality}}]
Let $F$ be a linearly ordered field.
Let $P$ be a linear program over $F_\infty$.
If $P$ reaches $p$ and the dual of $P$ reaches $q$,
then $p + q \ge 0$.
...
\begin{lemma}[\customlabel{infeasible-of-unbounded}{{\bf infeasible\_of\_unbounded}}]
...
Obtain contradiction using \ref{weakDuality:restated}.

\begin{lemma}[\customlabel{unbounded-of-reaches-le}{{\bf unbounded\_of\_reaches\_le}}]
Let $F$ be a linearly ordered field.
Let $P$ be a linear program over $F_\infty$.
...
\begin{lemma}[\customlabel{unbounded-of-feasible-of-neg}{{\bf unbounded\_of\_feasible\_of\_neg}}]
Let $F$ be a linearly ordered field.
...
\begin{lemma}[\customlabel{dualize-dualize}{{\bf dualize\_dualize}}]
...
\begin{lemma}[\customlabel{strongDuality-aux}{{\bf strongDuality\_aux}}]
...
\begin{lemma}[\customlabel{strongDuality-of-both-feasible}{{\bf strongDuality\_of\_both\_feasible}}]
Let $P$ be a linear program such that
$P$ is feasible and the dual of $P$ is also feasible.
...
\begin{lemma}[\customlabel{unbounded-of-feasible-of-infeasible}{{\bf unbounded\_of\_feasible\_of\_infeasible}}]
Let $P$ be a linear program such that
...
\begin{lemma}[\customlabel{optimum-unique}{{\bf optimum\_unique}}]
...
\begin{lemma}[\customlabel{optimum-eq-of-reaches-bounded}{{\bf optimum\_eq\_of\_reaches\_bounded}}]
Let $P$ be a linear program.
Let $r$ be a valued reached by $P$ such that $P$ is bounded by $r$.
...
\begin{lemma}[\customlabel{strongDuality-of-prim-feas}{{\bf strongDuality\_of\_prim\_feas}}]
...
Using \ref{optimum-eq-of-reaches-bounded} together with \ref{weakDuality:restated},
...
\begin{theorem}[\customlabel{optimum-neq-none}{{\bf optimum\_neq\_none}}]
...
\begin{lemma}[\customlabel{strongDuality-of-dual-feas}{{\bf strongDuality\_of\_dual\_feas}}]
...
\begin{theorem}[\customlabelrestated{strongDuality:restated}{{\bf strongDuality}}]
Let $F$ be a linearly ordered field.
Let $P$ be a linear program over $F_\infty$.
If $P$ or its dual is feasible (at least one of them),
then there exists $p$ in $F_\infty$ such that
