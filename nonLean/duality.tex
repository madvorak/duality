...
In Lean, it would have been be possible to prove \ref{equalityFarkas}
for reals using the Hahn-Banach separation theorem \cite{HahnBanach-Lean}:

Let $E$ be a topological vector space over reals.
Let $s$ be a closed convex set in $E$.
Let $x$ be an element of $E$. If $x \notin s$
then there exists a continuous linear map $f : E \r \mathbb{R}$
and a number $u : \mathbb{R}$ such that, for all $a \in s$,
$f~a < u < f~x$.

It would then have been possible to extend the result to other
duality theorems for reals (and for extended reals).
To our knowledge, the theorem \cite{HahnBanach-Lean} cannot be used
to prove \ref{equalityFarkas} for all linearly ordered fields.
