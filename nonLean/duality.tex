...
The following instance in Mathlib defines how explicit vectors are compared
(it is, in fact, more general than we said because it talks about $\Pi$-types
\cite{MLTT}, i.e., the \textit{type} of \texttt{x i} can depend on the
\textit{value} of \texttt{i}):
\begin{lstlisting}
instance Pi.hasLe {ι : Type u} {α : ι → Type v} [∀ i, LE (α i)] :
  LE (∀ i, α i) where le x y := ∀ i, x i ≤ y i
\end{lstlisting}
This way Mathlib naturally defines $x \le y$ to hold if and only if
