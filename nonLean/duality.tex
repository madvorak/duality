...
have the same optimal values {\color{blue} TODO reformulate this sentence.}
...
{\color{blue} TODO change in the code, then change here.}
\end{theorem}
...
\sekt{modules}). {\color{blue} TODO inline the (...) into theorem statement somehow.}
This result is due to \cite{Bartl2007} and our proof is based on
\cite{Bartl2011}.
...
If $P$ is unbounded then $P^\star=\bot$.
If there exists finite $r\in F$ such that $c \* x \ge r$ for all solutions $x$
and $c \* x^\star = r$ for some solution $x^\star$ then $P^\star=r$.
%(Clearly, these three cases are mutually exclusive, and such $r$, if exists, is unique).
If none of the cases holds then $P^\star$ is undefined.
}
\end{definition}
...
it is defined.
{\color{blue} TODO reconsider: Is $\restriction$ idiomatic?}
...
The strong duality holds. {\color{blue} TODO rephrase in terms of OppositesOpt.}
\end{lemma}
...
{\color{blue} TODO rephrase in terms of OppositesOpt.}
\end{theorem}
...
\subsection{Counterexamples for~\ref{extendedFarkas}}\label{sec:extendedFarkas:counterexamples}
Recall that~\ref{extendedFarkas} has four preconditions on matrix $A$ and vector $b$.
The following examples show that omitting any of these preconditions makes the theorem false: there may exist nonnegative vectors $x,y$ such that 
